...
document.addEventListener("DOMContentLoaded", () => {
    ...
    async function checkBackendHealth() {
        try {
            ...
            if (!response.ok)
                throw new Error("Помилка зв'язку з бекендом");
        }
        catch (error) {
    ...
    function validateForm() {
        ...
        if (!PHONE_REGEX.test(phoneVal) || phoneVal.length < 10 || phoneVal.length > 15) {
            if (phoneError)
                phoneError.textContent = "Формат: від 10 до 15 символів (+380XXXXXXXXX).";
        ...
        if (jobTitle) {
            if (jobTitle.length > 50)
                errors.push("Посада має бути до 50 символів.");
            const jobCompany = val("job-company");
            if (!jobCompany || jobCompany.length > 50)
                errors.push("Вкажіть компанію (до 50 символів).");
            const jobLoc = val("job-location");
            if (!jobLoc || jobLoc.length > 50)
                errors.push("Вкажіть локацію роботи (до 50 символів).");
        ...
        if (sciName) {
            if (sciName.length > 100)
                errors.push("Назва інтересу: максимум 100 символів.");
        ...
        if (pubTitle) {
            if (pubTitle.length > 200)
                errors.push("Назва публікації: максимум 200 символів.");
            const pubJournal = val("pub-journal");
            if (pubJournal && pubJournal.length > 100)
                errors.push("Журнал публікації: максимум 100 символів.");
        ...
        if (awardTitle) {
            if (awardTitle.length > 100)
                errors.push("Назва нагороди: максимум 100 символів.");
            const awardOrg = val("award-org");
            if (awardOrg && awardOrg.length > 100)
                errors.push("Організація нагороди: максимум 100 символів.");
    ...
    async function handleFormSubmit(e) {
        e.preventDefault();
        if (!validateForm() || !submitBtn || !form)
            return;
