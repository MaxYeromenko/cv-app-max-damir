...
document.addEventListener("DOMContentLoaded", () => {
    ...
    async function loadMemberData(): Promise<void> {
        try {
            ...
            if (!response.ok) throw new Error("Резюме не знайдено");

            const cvData: UserCV = await response.json();
            populateDOM(cvData);
    ...
    function populateDOM(cv: UserCV): void {
        ...
        const photoEl = document.getElementById("cv-photo") as HTMLImageElement | null;
        if (photoEl) {
        ...
        const emailEl = document.getElementById("cv-email") as HTMLAnchorElement | null;
        if (emailEl) {
            emailEl.href = `mailto:${pInfo.email}`;
            emailEl.textContent = pInfo.email;
        }

        const phoneEl = document.getElementById("cv-phone") as HTMLAnchorElement | null;
        if (phoneEl) {
            phoneEl.href = `tel:${pInfo.phone}`;
            phoneEl.textContent = pInfo.phone;
        }

        const dobEl = document.getElementById("cv-dob") as HTMLTimeElement | null;
        if (dobEl) {
        ...
        setElementText(
            "cv-hours",
            cv.device_access?.weekly_hours ? `${cv.device_access.weekly_hours} год/тижд` : "Гнучкий графік"
        );

        renderList("cv-languages", cv.languages, (lang) => {
            const level = PROFICIENCY_LABELS[lang.proficiency as ProficiencyLevel] || lang.proficiency;
        ...
        renderCommaList("cv-programming-skills", cv.programming_skills, (skill) => {
            const level = PROGRAMMING_LEVEL_LABELS[skill.level as ProgrammingLevel] || skill.level;
        ...
        renderCommaList("cv-subjects", cv.favorite_subjects_in_school, (sub) => {
            return SUBJECT_LABELS[sub as SchoolSubject] || sub;
        });
